    ...
    class RickshawOnPolicyRunner(base_runner_type):
        ...
        def __init__(self, *args: Any, **kwargs: Any) -> None:
            ...
            if training_configuration is not None:
                ...
                if int(self.alg.actor.history_length) != int(parameters["history_length"]):
        ...
        def load(self, path: str, load_cfg=None, strict: bool = True, map_location=None):
            ...
            if loaded_stage in {"s0_teacher", "s2_student_ppo"}:
                ...
                # A playback runner can use a different local rollout length
                # than the checkpoint.  The curriculum ABI is defined by the
                # producer's completed policy samples, not the viewer config.
                saved_rollout_steps = int(
                    loaded_configuration["training_parameters"]["rollout_steps"]
                )
                self._g1_stage_policy_steps = (
                    self._g1_training_iterations * saved_rollout_steps
                )
