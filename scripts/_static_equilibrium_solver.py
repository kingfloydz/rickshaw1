"""Offline solver for generating the certified flat-ground rest pose."""
...
from g1_rickshaw_lab.static_equilibrium import MujocoStaticEquilibrium
...
def fixed_contact_static_components(
    *,
    ...
    tuple[tuple[Any, ...], tuple[Any, ...]], tuple[tuple[Any, ...], tuple[Any, ...]]
]:
    ...
    hand_normal_total = (
        handle_n * hand_tangent_total + gravity_pitch_moment
    ) / handle_s
...
def solve_fixed_contact_statics(
    *,
    ...
    force_residual = tuple(
        gravity_force[axis]
        + sum(force[axis] for force in hand_forces)
    ...
    for lateral, force in zip(
        (hitch_half_width, -hitch_half_width), hand_forces, strict=True
    ):
    ...
    for lateral, force in zip(
        (0.5 * wheel_track, -0.5 * wheel_track), wheel_forces, strict=True
    ):
...
def _nominal_qpos(model: Any) -> np.ndarray:
    ...
    for joint_id in range(model.njnt):
        ...
        qpos[int(model.jnt_qposadr[joint_id])] = (
            G1_DEFAULT_LOWER_WAIST_JOINT_POSITIONS.get(
    ...
    grasp_midpoint = 0.5 * (
        data.site("robot/left_grasp_site").xpos
        + data.site("robot/right_grasp_site").xpos
    )
...
def _pose_multistart_seeds(
    ...
    for index in range(count):
        ...
        if profile:
            ...
            left_delta = (
                (-1.0 if profile % 2 == 0 else 1.0) * arm_scale * rng.standard_normal(7)
            )
            seed[joint_start : joint_start + 7] += left_delta
            seed[joint_start + 7 : joint_start + 14] += right_symmetry * left_delta
        seeds.append(
            np.clip(seed, np.nextafter(lower, upper), np.nextafter(upper, lower))
        )
    return tuple(seeds)
...
def solve_mujoco_static_equilibrium(
    ...
    *,
    ...
    robot_joint_q = np.array(
        [model.jnt_qposadr[index] for index in robot_joint_ids], dtype=int
    )
    robot_joint_v = np.array(
        [model.jnt_dofadr[index] for index in robot_joint_ids], dtype=int
    )
    joint_names = [
        mujoco.mj_id2name(model, mujoco.mjtObj.mjOBJ_JOINT, index) or ""
        for index in robot_joint_ids
    ]
    if tuple(name.removeprefix("robot/") for name in joint_names) != G1_JOINT_ORDER:
        raise ValueError(
            "MuJoCo robot joint order does not match the fixed policy order"
        )
    ...
    if robot_actuator_ids.size not in (0, len(robot_joint_ids)):
        raise ValueError(
            "static model must have either zero or one actuator per robot joint"
        )
    wheel_v = np.array(
        [
    ...
    for body_name in ("robot/left_ankle_roll_link", "robot/right_ankle_roll_link"):
        ...
        foot_geoms.extend(
            ...
            if model.geom_type[index] == mujoco.mjtGeom.mjGEOM_SPHERE
        )
    wheel_geoms = [
    ...
    for offset, joint_id in enumerate(arm_joint_ids):
        if model.jnt_limited[joint_id]:
            lower[joint_start + offset], upper[joint_start + offset] = model.jnt_range[
                joint_id
            ]
    ...
    def kinematic_residuals() -> tuple[
        ...
        foot_height = np.array(
            [
                np.mean(
                    [
                        data.geom_xpos[index, 2] - model.geom_size[index, 0]
                        for index in foot_geoms[offset : offset + 4]
                    ]
                )
                for offset in (0, 4)
            ]
    ...
    def hitch_height_violation(hitch_height: float) -> float:
        ...
        return hitch_height - float(
            np.clip(
                hitch_height,
                low + cfg.hitch_height_margin,
                high - cfg.hitch_height_margin,
            )
        )
    ...
    def static_external_forces() -> tuple[np.ndarray, np.ndarray]:
        ...
        for side, cart_force in zip(
            ("left", "right"), static_cart.handle_forces_sln, strict=True
        ):
        ...
        for body_name, wheel_force in zip(
            ...
            external += point_force_jacobian(
                body.id, np.asarray(body.xpos)
            ) @ np.asarray(wheel_force)
        ...
        foot_force_map = np.concatenate(
            [
                point_force_jacobian(
                    ...
                    - np.array((0.0, 0.0, model.geom_size[geom_id, 0])),
                )
        ...
        root_target = (
            ...
            - external[robot_root_v : robot_root_v + 6]
        )
        foot_forces = np.linalg.lstsq(
            foot_force_map[robot_root_v : robot_root_v + 6], root_target, rcond=None
        )[0]
    ...
    def residual(x: np.ndarray) -> np.ndarray:
        ...
        position_error, support_error, foot_tilt, torso_pitch, hitch_height = (
            kinematic_residuals()
        )
        ...
        torque_barrier = (
            np.logaddexp(0.0, 20.0 * (torque_ratio - cfg.torque_barrier_fraction)) / 2.0
        )
        return np.concatenate(
            (
                ...
                np.asarray(
                    (hitch_height_violation(hitch_height) / cfg.hitch_height_scale,)
                ),
    ...
    try:
        ...
        ranked_seeds = sorted(
            initial_seeds, key=lambda seed: float(np.linalg.norm(residual(seed)))
        )
        for seed in ranked_seeds[: cfg.screening_candidates]:
            ...
            unpack(candidate.x)
            mujoco.mj_forward(model, data)
            position_error, support_error, foot_tilt, _, hitch_height = (
                kinematic_residuals()
            )
            data.qacc[:] = 0.0
            mujoco.mj_inverse(model, data)
            ...
            constraint_error = max(
                ...
                float(
                    np.linalg.norm(candidate_force[unactuated_v], ord=np.inf)
                    / cfg.unactuated_force_scale
                ),
                float(
                    max(0.0, -np.min(candidate_foot_forces[:, 2]))
                    / cfg.support_force_tolerance
                ),
            )
            torque_ratio = float(
                np.max(np.abs(candidate_force[robot_joint_v]) / effort_limits)
            )
            candidates.append(
                (
                    (
        ...
        def evaluate_candidate(
                ...
                *,
                ...
                def refine_residual(x: np.ndarray) -> np.ndarray:
                    ...
                    terms = [
                        ...
                        np.asarray(
                            (
                                hitch_height_violation(hitch_height)
                                / cfg.hitch_height_scale,
                            )
                        ),
                        (
                            x[joint_start:joint_end]
                            - constraint_seed[joint_start:joint_end]
                        )
                        / posture_scale,
                    ]
                    excess = np.maximum(
                ...
                def absolute_jacobian(x: np.ndarray) -> np.ndarray:
                    ...
                    for index in range(x.size):
                        ...
                        if shifted[index] == x[index]:
                            shifted[index] = max(
                                x[index] - difference_step, constraint_lower[index]
                            )
                        jacobian[:, index] = (refine_residual(shifted) - value) / (
            ...
            unactuated_error = float(
                np.linalg.norm(inverse_force[unactuated_v], ord=np.inf)
            )
            ...
            wheel_normal_error = float(
                max(
                    0.0,
                    -min(force[2] for force in static_cart.wheel_contact_forces_sln),
                )
            )
            contact_force_error = max(
                foot_normal_error, foot_friction_error, wheel_normal_error
            )
            ...
            diagnostics = {
                ...
                "torque_joint": joint_names[
                    int(np.argmax(np.abs(joint_torque) / effort_limits))
                ],
