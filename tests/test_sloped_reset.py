...
from g1_rickshaw_lab.tasks.manager_based.rickshaw_velocity.closed_chain import build_assembled_spec
...
def test_sloped_reset_keeps_g1_root_upright_and_only_changes_ankle_pitch() -> None:
    ...
    flat_root_quaternion = np.broadcast_to(flat[robot_root + 3 : robot_root + 7], (len(TERRAIN_SLOPES), 4))
    np.testing.assert_allclose(templates.robot_root_pose[:, 3:7], flat_root_quaternion)
    flat_root_xy = np.broadcast_to(flat[robot_root : robot_root + 2], (len(TERRAIN_SLOPES), 2))
    np.testing.assert_allclose(templates.robot_root_pose[:, :2], flat_root_xy)
    assert np.max(np.abs(templates.robot_root_pose[:, 2] - flat[robot_root + 2])) < 1.1e-3
    for joint_index, joint_name in enumerate(G1_JOINT_ORDER):
        ...
        np.testing.assert_allclose(templates.robot_joint_position[:, joint_index], expected)
...
def test_sloped_reset_aligns_feet_and_wheels_without_breaking_hitches() -> None:
    ...
    joint_qpos = tuple(int(model.joint(f"robot/{name}").qposadr[0]) for name in G1_JOINT_ORDER)
    foot_geoms = tuple(
        geom_id
    ...
    wheel_bodies = tuple(model.body(f"rickshaw/{side}_wheel_link").id for side in ("left", "right"))

    flat_data = mujoco.MjData(model)
    flat_data.qpos[:] = flat
    mujoco.mj_forward(model, flat_data)
    flat_foot_clearance = np.mean(
        [flat_data.geom_xpos[geom_id, 2] - model.geom_size[geom_id, 0] for geom_id in foot_geoms]
    )
    flat_wheel_clearance = np.mean([flat_data.xpos[body_id, 2] - WHEEL_RADIUS for body_id in wheel_bodies])
    ...
    for slope_index, slope in enumerate(TERRAIN_SLOPES):
        ...
        wheel_clearance = np.array([normal @ data.xpos[body_id] - WHEEL_RADIUS for body_id in wheel_bodies])
        hitch_error = max(
            np.linalg.norm(
                data.site(f"robot/{side}_grasp_site").xpos - data.site(f"rickshaw/{side}_hitch_site").xpos
            )
            for side in ("left", "right")
        )

        np.testing.assert_allclose(np.mean(foot_clearance), flat_foot_clearance, atol=1.0e-10)
        np.testing.assert_allclose(np.mean(wheel_clearance), flat_wheel_clearance, atol=1.0e-10)
